...
export function RequesterTicketForm({
  ...
  if (isContextLoading) {
    return (
      <section className="rounded-panel border border-border-subtle bg-surface-1 p-lg shadow-panel">
  ...
  if (isContextError || !requesterContext) {
    return (
      <section className="rounded-panel border border-danger/30 bg-danger-bg p-lg shadow-panel">
        <div className="flex items-start gap-sm">
          <IconAlertTriangle size={20} className="mt-1 shrink-0 text-danger" />
          <div>
            <h2 className="text-h3 font-semibold text-danger">
              Yardım formu yüklenemedi
            </h2>
            ...
            <button
              ...
              className="mt-md inline-flex items-center gap-xs rounded-xl border border-danger/30 bg-surface-1 px-md py-sm text-body font-medium text-danger transition hover:bg-danger-bg focus:outline-none focus:ring-2 focus:ring-danger/25"
            >
  ...
  return (
    <section className="rounded-panel border border-border-subtle bg-surface-1 p-lg shadow-panel">
      <div>
        ...
        <h2 className="mt-xs text-h2 font-semibold text-text-primary">
          Nasıl yardımcı olabiliriz?
        </h2>
        <p className="mt-sm text-body text-text-secondary">
      ...
      {requesterContext.approval_preview.requires_approval ? (
        <div className="mt-md rounded-2xl border border-warning/30 bg-warning-bg p-md text-body font-medium text-warning">
      ...
      {successState ? (
        <div className="mt-md rounded-2xl border border-success/30 bg-success-bg p-md text-body font-medium text-success">
          <div className="flex gap-sm">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-xs">
                {successState.ticket.approval_status === "pending"
                  ? `Önce ${
                      successState.ticket.pending_approver_name ?? "yöneticin"
                    } onayına gidecek.`
      ...
      <form className="mt-lg space-y-lg" onSubmit={handleSubmit}>
        <section>
            ...
            {requesterCategoryOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-2xl border p-md text-left shadow-sm transition focus:outline-none focus:ring-2 focus:ring-accent/20 ${
                    isSelected
                      ? "border-accent bg-accent-bg text-accent"
                      : "border-border-subtle bg-surface-0 text-text-primary hover:border-accent hover:bg-accent-bg"
                  }`}
                >
                  <span className="text-2xl" aria-hidden={true}>
                    {option.icon}
                  </span>
                  <span className="mt-sm block font-semibold">
                    {option.label}
                  </span>
          ...
          <div className="mt-sm rounded-2xl border border-border-subtle bg-surface-0 p-md text-caption text-text-secondary">
        ...
        <section>
          ...
          <div className="mt-sm space-y-sm">
            <label
              className={`flex cursor-pointer items-start gap-sm rounded-2xl border p-md shadow-sm transition hover:border-accent hover:bg-accent-bg ${
                selectedAssetId === ""
            ...
            {requesterContext.active_assignments.map((assignment) => {
              const isSelected =
                selectedAssetId === String(assignment.asset_id);
            ...
            {requesterContext.active_assignments.length === 0 ? (
              <div className="rounded-2xl border border-border-subtle bg-surface-0 p-md text-caption text-text-secondary">
        ...
        <section className="grid gap-md">
          <label>
            ...
            <input
              ...
              className="mt-xs w-full rounded-2xl border border-border bg-surface-0 px-md py-sm text-body text-text-primary outline-none transition placeholder:text-text-muted focus:border-accent focus:ring-2 focus:ring-accent/20"
          ...
          <label>
            ...
            <textarea
              ...
              className="mt-xs min-h-[130px] w-full rounded-2xl border border-border bg-surface-0 px-md py-sm text-body text-text-primary outline-none transition placeholder:text-text-muted focus:border-accent focus:ring-2 focus:ring-accent/20"
        ...
        <section>
          ...
          <label
            ...
            onDrop={handleDrop}
            className="mt-sm flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-surface-0 p-lg text-center transition hover:border-accent hover:bg-accent-bg focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/20"
          >
            ...
            <div className="mt-sm space-y-xs">
              {files.map((file, index) => (
                <div
                  ...
                  className="flex items-center justify-between gap-sm rounded-2xl border border-border-subtle bg-surface-0 px-md py-sm text-caption text-text-secondary"
                >
                  ...
                  <button
                    ...
                    className="rounded-full p-xs text-text-secondary transition hover:bg-danger-bg hover:text-danger focus:outline-none focus:ring-2 focus:ring-danger/25"
        ...
        <button
          ...
          className="w-full rounded-2xl bg-accent px-md py-sm text-body font-semibold text-white shadow-sm transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-accent/25 disabled:cursor-not-allowed disabled:opacity-60"
        >
